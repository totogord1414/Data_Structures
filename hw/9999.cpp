...
int solve(vector<int>& a) {
    ...
    int global_max_sum = LVS[0][n-1]; // 全局最优解（预存只有 S1 没有 S2 的退化情况）
    ...
    for (int l2 = 1; l2 < n; ++l2) {
        ...
        for (int r = l2; r <= R_max; ++r) {
            int val = a[r];
            for (int p : pos[val]) {
                if (p < prefix_len) is_obstacle[p] = true;
                else break; // 由于 pos 是升序的，可以直接断开
            }
        }
        ...
        for (int r2 = R_max; r2 >= l2; --r2) {
            ...
            for (int p : pos[val]) {
                if (p < prefix_len) {
                    ...
                    if (p > 0 && !is_obstacle[p-1]) dsu.unite(p, p-1);
                    if (p + 1 < prefix_len && !is_obstacle[p+1]) dsu.unite(p, p+1);
